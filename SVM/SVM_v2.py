...
def maxDEi(dataMat, labelMat, alphas, indexI):
    gXi = gX(dataMat, labelMat, alphas, indexI)
    Ei_g = gXi - labelMat[indexI]
    maxDEi = 0

    indexJ = indexI
    for j in range(len(alphas)):
        gXj = gX(dataMat, labelMat, alphas, j)
        Ej_g = gXj - labelMat[j]
        DEi = Ei_g - Ej_g
        if abs(DEi) > abs(maxDEi):
            maxDEi = DEi
            indexJ = j

    return indexJ, maxDEi

def maxDis(dataMat, labelMat, alphas, indexI):
    maxDis2 = 0
    indexJ = indexI
    i = indexI
    for j in range(len(alphas)):
        disX1 = abs(dataMat[i][0] - dataMat[j][0])
        disX2 = abs(dataMat[i][1] - dataMat[j][1])
        dis2 = disX1**2 + disX2**2
        if dis2 > maxDis2:
            maxDis2 = dis2
            indexJ = j
    gXi = gX(dataMat, labelMat, alphas, indexI)
    Ei_g = gXi - labelMat[indexI]
    gXj = gX(dataMat, labelMat, alphas, indexJ)
    Ej_g = gXj - labelMat[indexJ]
    maxDEi = Ei_g - Ej_g
    return indexJ, maxDEi
...
def smoSimple(dataMatIn, classLabels, C, maxIter):
    ...
    while(iter_num < maxIter):
        for i in range(m):
            ...
            '''
            #j, deltaEi = maxDEi(dataMat, labelMat, alphas, i)
            j, deltaEi = maxDis(dataMat, labelMat, alphas, i)
            print('i,j:', i,j)
            ...
            alphaj_new = alphaj_old + yj*deltaEi/eta
