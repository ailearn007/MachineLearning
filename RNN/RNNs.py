# -*- coding: utf-8 -*-
# 本程序通过基本RNN模块训练恐龙名字生成器
# training set 为恐龙名字文本 dinos.txt
# 训练后模型可通过抽样生成新的恐龙名字
import numpy as np
...
def sample(parameters, char_to_ix, ix_to_char, seed = 1):
    Wax, Waa, ba, Wya, by = parameters['Wax'], parameters['Waa'], \
                            parameters['ba'], parameters['Wya'],parameters['by']
    ...
    while (counter != 50 and idx != newline_char):
        ...
        idx = np.random.choice(list(range(vocab_size)), p = np.squeeze(y)) #p must be 1-D array
        # idx = np.random.choice(list(range(vocab_size)), p = y[:,0].ravel())
        #经测试上式中使用 y.ravel()编译不通过，而使用y[:,0]（shape(n_y,)）编译通过
        indices.append(idx)
    ...
    sample_name = ''.join(ix_to_char[i] for i in indices)
    sample_name = sample_name[0].upper() + sample_name[1:]
    return sample_name
...
if __name__ == '__main__':
    ...
    parameters = model(char_to_ix, \
                       num_iter = 55000, n_a = 50, dino_names = 7, vocab_size = len(char_to_ix), learning_rate = 0.01)

    for i in range(10):
        sample_name = sample(parameters, char_to_ix, ix_to_char, i+10)
        print('Name: {0:50}'.format(sample_name))
