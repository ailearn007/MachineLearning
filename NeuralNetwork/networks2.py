...
def predict(X, parameters, sigma, mu):
    '''
...
if __name__ == '__main__':
    ...
    mu = train_X.mean(axis = 1, keepdims=True)
    sigma = train_X.std(axis = 1, keepdims=True)
    Y_predict_train = predict(train_X, parameters, sigma, mu)
    error_rate_train = np.mean(np.abs(Y_predict_train - train_Y))
    print('Train error rate is {0:.2%}'.format(error_rate_train))

    Y_predict_test = predict(test_X, parameters, sigma, mu)
