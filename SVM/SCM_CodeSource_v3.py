...
def testRbf(k1 = 1.3):
    ...
    Returns:
        无
    """
    dataArr,labelArr = loadDataSet('nonlinearset.txt')                        #加载训练集
    b,alphas = smoP(dataArr, labelArr, 200, 0.0001, 100, ('rbf',1.3))        #根据训练集计算b和alphas
    ...
    dataArr,labelArr = loadDataSet('nonlinearset.txt')                         #加载测试集
    errorCount = 0
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()
    m,n = np.shape(datMat)
    for i in range(m):
