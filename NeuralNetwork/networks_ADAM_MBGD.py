...
def compute_mini_batches(shuffled_X, shuffled_Y, batch_size):
...
def MBGD_model(X, Y, layer_dims, learning_rate, max_epoch, batch_size, print_cost = False):
    X = normalize_input(X)
    parameters = initialize_paras(layer_dims)
    Vd, Sd = initialize_ADAM(parameters) #ADAM 初始化
    costs = []

    shuffled_X, shuffled_Y = dataShuffle(X, Y)
    batches = compute_mini_batches(shuffled_X, shuffled_Y, batch_size)
    for epoch_num in range(1, 1+max_epoch):
        for mini_batch in batches:
            ...
            parameters, Vd, Sd = update_parameters_ADAM(parameters, grads, learning_rate, \
                                                        epoch_num, Vd, Sd, beta_1=0.9, beta_2=0.999, epsilon=1e-8)
        if epoch_num%100 == 0:
            costs.append(cost)
            if print_cost:
                print('[Epoch {0:^6} Cost = {1:8.6}]'.format(epoch_num, cost))

    plt.plot(np.squeeze(costs))
    plt.xlabel('epoches (per hundred)')
    plt.ylabel('cost')
...
if __name__ == '__main__':
    ...
    max_epoch = 10000
    #batch_size = X.shape[1] #BGD
    batch_size = 2**7
    #batch_size = 1 #SGD
    parameters = MBGD_model(X, Y, layer_dims, learning_rate, max_epoch, batch_size, print_cost = True)
